...
| **Obsidian CLI** | **Yes — always** | No (core feature, opt-in) | First-party. Vault-aware: backlinks, orphans, tags, Bases queries. Requires 1.12.7+ and a running desktop app (Source: [[obsidian-cli-official-page]]). |
...
**The environment qualifier is load-bearing.** The CLI requires a running desktop app for every command, so the rule resolves differently by context: on a workstation the CLI is the best available transport; in CI it is not a transport at all. Headless automation falls to filesystem, optionally fed by [[obsidian-headless]] over Sync. See [[Headless Agent Automation]].
